...
#  SQLALCHEMY_DATABASE_URI = 'postgresql+psycopg2://appuser:pass123@localhost/appdb'
...
def register():
    ...
    if form.validate_on_submit():
        ...
        if (form.email.data =='employee@mail.com'):
            ...
            db.session.commit()
        return redirect("https://www.public-services.site/api/login/")
    else:
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, form.password.data):
            ...
            if current_user.has_roles('citizen'):
                print('hasrolecitizen')
                return redirect("https://www.public-services.site/api/citizen")                                      #---------------------Vasilis
#                return redirect("http://localhost:5001/api/citizen")
            elif current_user.has_roles('employee'):
                print('hasroleemployee')
                return redirect("https://www.public-services.site/api/employee")                                     #---------------------Vasilis
#                return redirect("http://localhost:5001/api/employee")

            #return redirect("http://localhost:5001/api/menu")
     #       return redirect("https://www.public-services.site/app/menu")   
        #redirect(url_for('profile'))
        else:
            print('error')
            flash('Incorrect username or password. Try again.')
            redirect("https://www.public-services.site/api/login/")                                                                    
...
if __name__ == "__main__":
    app.run(debug=True,host='0.0.0.0', port=5000)                                                            #---------------------Vasilis

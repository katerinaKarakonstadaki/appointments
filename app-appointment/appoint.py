...
class Configuration(object):
  ...
  SQLALCHEMY_DATABASE_URI = 'postgresql+psycopg2://appuser:pass123@psql-db/appdb'                    #---------------------Vasilis
  POSTGRES_SERVER='psql-db'
...
private_key= 'Thisisaprivatekey'
public_key = 'Thisisapublickey'
app.config['SQLALCHEMY_DATABASE_URI']='postgresql+psycopg2://appuser:pass123@psql-db/appdb'         #---------------------Vasilis
app.config['POSTGRES_SERVER']='psql-db'                                                           #---------------------Vasilis
...
def appointment_create():
    if not current_user.is_authenticated:
        return redirect('https://www.public-services.site/api/login')                                          #---------------------Vasilis
#        return redirect('http://127.0.0.1:5000/api/login') 
    ...
    if request.method == 'POST' and form.validate():
        ...
        db.session.add(apt)
        db.session.commit()
        flash('appointment is successfully created!')
        return redirect('https://www.public-services.site/api/citizen')
    #else:
        #print(current_user)
        #return redirect('http://127.0.0.1:5000/api/login')
...
#        return redirect('http://127.0.0.1:5000/api/login')
    ...
    conn = psycopg2.connect("dbname='appdb' user='appuser' host='psql-db' password='pass123'")   #----------------------Vasilis
    ...
    if request.method == 'POST':
        if request.form['submit-button'] == "Submit":
            for checkbox in request.form.getlist('check'):
                ...
                conn.commit()
                res = requests.post('http://flask-notify-clip:5000/api/notify/', json={"email":checkbox})       #--------------------------Vasilis
...
def logout_appoint():
    logout_user()
    return redirect('https://www.public-services.site/api/home')                                                  #--------------------------Vasilis
...
if __name__ == '__main__':
    app.run(debug=True,host='0.0.0.0')                                               #--------------------------Vasilis
